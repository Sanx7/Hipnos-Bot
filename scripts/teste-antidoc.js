// ============================================
// 🧪 teste-antidoc.js — Permissão do /antidoc (pós-correção LID)
// ============================================
...
const CHAVE = 'antiDocument'
const RECUSA = /pergaminhos deste reino/
const USO = /barrar arquivos e documentos/
...
async function main () {
  ...
  await testar('exports: nome antidoc e executar', () => {
    if (cmd.nome !== 'antidoc') throw new Error('nome errado: ' + cmd.nome)
  ...
  await testar('admin autorizado → toggle 1 GRAVA e 0 REMOVE (função sensível)', async () => {
    ...
    try {
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antidoc 1')
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antidoc 0')
  ...
  await testar('dono do bot não-admin autorizado → vê o uso', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${DONO}@s.whatsapp.net`), '/antidoc')
  ...
  await testar('não-admin/não-dono → recusa temática, sem efeito', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${INTRUSO}@s.whatsapp.net`), '/antidoc 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid de ADMIN (grupo LID) bate pelo id → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_ID}@lid`), '/antidoc')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid do ADMIN resolve via SESSÃO (lid.js) → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_SESSAO}@lid`), '/antidoc')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid não resolvível de intruso → recusa', async () => {
    const { sock, enviadas } = criarSock([p(`${ADMIN}@s.whatsapp.net`, 'admin'), p(`${INTRUSO}@s.whatsapp.net`, null)])
    await cmd.executar(sock, JID, msgDe(`${LID_DESCONHECIDO}@lid`), '/antidoc 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('groupMetadata falho → aviso amigável, nada escapa', async () => {
    ...
    let lancou = false
    try { await cmd.executar(sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antidoc 1') } catch (e) { lancou = true }
