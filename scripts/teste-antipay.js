// ============================================
// 🧪 teste-antipay.js — Permissão do /antipay (pós-correção LID)
// ============================================
...
const CHAVE = 'antiPayment'
const RECUSA = /transações mundanas/
const USO = /banir cobranças materiais/
...
async function main () {
  ...
  await testar('exports: nome antipay e executar', () => {
    if (cmd.nome !== 'antipay') throw new Error('nome errado: ' + cmd.nome)
  ...
  await testar('admin autorizado → toggle 1 GRAVA e 0 REMOVE (função sensível)', async () => {
    ...
    try {
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antipay 1')
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antipay 0')
  ...
  await testar('dono do bot não-admin autorizado → vê o uso', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${DONO}@s.whatsapp.net`), '/antipay')
  ...
  await testar('não-admin/não-dono → recusa temática, sem efeito', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${INTRUSO}@s.whatsapp.net`), '/antipay 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid de ADMIN (grupo LID) bate pelo id → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_ID}@lid`), '/antipay')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid do ADMIN resolve via SESSÃO (lid.js) → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_SESSAO}@lid`), '/antipay')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid não resolvível de intruso → recusa', async () => {
    const { sock, enviadas } = criarSock([p(`${ADMIN}@s.whatsapp.net`, 'admin'), p(`${INTRUSO}@s.whatsapp.net`, null)])
    await cmd.executar(sock, JID, msgDe(`${LID_DESCONHECIDO}@lid`), '/antipay 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('groupMetadata falho → aviso amigável, nada escapa', async () => {
    ...
    let lancou = false
    try { await cmd.executar(sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antipay 1') } catch (e) { lancou = true }
