// ============================================
// 🧪 teste-antistatus.js — Permissão do /antistatus (pós-correção LID)
// ============================================
...
// Uso: node scripts/teste-antistatus.js
// ============================================
process.env.OWNER_NUMBERS = '5511999990009' // ANTES do require (config lê no load)

const fs = require('fs')
const path = require('path')
const cmd = require('../comandos/admin/antistatus')
...
const CHAVE = 'antiStatus'
const RECUSA = /quais visões cruzam este reino/
const USO = /banir marcações de status/
...
const msgDe = (participant) => ({
  ...
  pushName: 'Autor',
  message: { conversation: '/antistatus' }
})
...
async function main () {
  ...
  await testar('exports: nome antistatus e executar', () => {
    if (cmd.nome !== 'antistatus') throw new Error('nome errado: ' + cmd.nome)
  ...
  await testar('admin autorizado → toggle 1 GRAVA e 0 REMOVE (função sensível)', async () => {
    ...
    try {
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antistatus 1')
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antistatus 0')
  ...
  await testar('dono do bot não-admin autorizado → vê o uso', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${DONO}@s.whatsapp.net`), '/antistatus')
  ...
  await testar('não-admin/não-dono → recusa temática, sem efeito', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${INTRUSO}@s.whatsapp.net`), '/antistatus 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid de ADMIN (grupo LID) bate pelo id → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_ID}@lid`), '/antistatus')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid do ADMIN resolve via SESSÃO (lid.js) → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_SESSAO}@lid`), '/antistatus')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid não resolvível de intruso → recusa', async () => {
    const { sock, enviadas } = criarSock([p(`${ADMIN}@s.whatsapp.net`, 'admin'), p(`${INTRUSO}@s.whatsapp.net`, null)])
    await cmd.executar(sock, JID, msgDe(`${LID_DESCONHECIDO}@lid`), '/antistatus 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('groupMetadata falho → aviso amigável, nada escapa', async () => {
    ...
    let lancou = false
    try { await cmd.executar(sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antistatus 1') } catch (e) { lancou = true }
