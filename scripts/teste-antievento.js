// ============================================
// 🧪 teste-antievento.js — Permissão do /antievento (pós-correção LID)
// ============================================
...
// Uso: node scripts/teste-antievento.js
// ============================================
process.env.OWNER_NUMBERS = '5511999990009' // ANTES do require (config lê no load)

const fs = require('fs')
const path = require('path')
const cmd = require('../comandos/admin/antievento')
...
const CHAVE = 'antiEvent'
const RECUSA = /linhas do tempo deste reino/
const USO = /barrar a criação de eventos/
...
const msgDe = (participant) => ({
  ...
  pushName: 'Autor',
  message: { conversation: '/antievento' }
})
...
async function main () {
  ...
  await testar('exports: nome antievento e executar', () => {
    if (cmd.nome !== 'antievento') throw new Error('nome errado: ' + cmd.nome)
  ...
  await testar('admin autorizado → toggle 1 GRAVA e 0 REMOVE (função sensível)', async () => {
    ...
    try {
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antievento 1')
      ...
      await cmd.executar(r.sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antievento 0')
  ...
  await testar('dono do bot não-admin autorizado → vê o uso', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${DONO}@s.whatsapp.net`), '/antievento')
  ...
  await testar('não-admin/não-dono → recusa temática, sem efeito', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${INTRUSO}@s.whatsapp.net`), '/antievento 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid de ADMIN (grupo LID) bate pelo id → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_ID}@lid`), '/antievento')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid do ADMIN resolve via SESSÃO (lid.js) → autorizado', async () => {
    ...
    await cmd.executar(sock, JID, msgDe(`${LID_SESSAO}@lid`), '/antievento')
    const texto = textoUnico(enviadas)
  ...
  await testar('sender @lid não resolvível de intruso → recusa', async () => {
    const { sock, enviadas } = criarSock([p(`${ADMIN}@s.whatsapp.net`, 'admin'), p(`${INTRUSO}@s.whatsapp.net`, null)])
    await cmd.executar(sock, JID, msgDe(`${LID_DESCONHECIDO}@lid`), '/antievento 1')
    const texto = textoUnico(enviadas)
  ...
  await testar('groupMetadata falho → aviso amigável, nada escapa', async () => {
    ...
    let lancou = false
    try { await cmd.executar(sock, JID, msgDe(`${ADMIN}@s.whatsapp.net`), '/antievento 1') } catch (e) { lancou = true }
