...
module.exports = {
  nome: 'play',
  descricao: 'Pesquisa e baixa uma música do YouTube usando APIs alternativas.',
  async executar(sock, jid, msg, texto) {
    try {
      ...
      // Lista de APIs espelhadas prontas para uso caso alguma caia
      const apis = [
      ...
      for (const api of apis) {
        try {
          ...
          // Extrai o link dependendo da estrutura de resposta de cada API padrão
          const data = response.data
          downloadUrl = data.downloadUrl || data.result?.download || data.result?.url || data.data?.url
          
          if (downloadUrl) break // Conseguiu o link de download, sai do loop
        } catch (e) {
