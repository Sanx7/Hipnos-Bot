...
module.exports = {
  ...
  async executar(sock, jid, msg, texto) {
    try {
      ...
      // Fazendo a requisição para uma instância oficial/estável do Cobalt
      ...
      // Baixando o arquivo final gerado pelo Cobalt
      ...
      writer.on('error', async (err) => {
        console.error(err)
        await sock.sendMessage(jid, { text: '❌ Erro ao salvar o áudio no servidor.' }, { quoted: msg })
        if (fs.existsSync(arquivoSaida)) fs.unlinkSync(arquivoSaida)
      })

    } catch (err) {
      console.error('Erro geral no comando play:', err)
      await sock.sendMessage(jid, { text: '❌ O servidor de download está sobrecarregado. Tente novamente em instantes.' }, { quoted: msg })
    }
  }
}