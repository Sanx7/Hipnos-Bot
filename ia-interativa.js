...
// ⏳ COOLDOWN: 1 resposta a cada 30s POR USUÁRIO (rpg/cooldown.js, em memória);
...
const COOLDOWN_MS = 30 * 1000       // ⏳ requisito: 30s por usuário
...
//   4) cooldown de 30s por usuário (em cooldown → silêncio).
...
async function processarGatilhoIA (sock, jid, msg, texto) {
  try {
    ...
    // 4) Cooldown de 30s POR USUÁRIO (em cooldown → silêncio total)
    ...
    //    citando a mensagem do gatilho
    const resposta = chamarIATeste
      ? await chamarIATeste(mensagem)
      : await chamarIA(mensagem)
